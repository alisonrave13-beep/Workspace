// ============================================================
//  🛠️ COMPONENTE PARA COMPLETAR — TAREA ESTUDIANTE
//  WhyUs.jsx  (¿Por qué elegirnos?)
//
//  INSTRUCCIONES:
//  1. Define el array "razones" con al menos 4 objetos.
//     Cada objeto debe tener: icono, titulo, descripcion.
//  2. Agrega una imagen en la columna izquierda.
//  3. Escribe 2 párrafos sobre la empresa.
//  4. Usa .map() para renderizar la lista de razones.
//
//  PISTAS:
//  - <div className="row align-items-center"> para las columnas
//  - className="list-unstyled" en <ul> quita los bullets
//  - Placeholder: https://placehold.co/500x400/1a1e2e/f5c518?text=VoltTec
// ============================================================

// 🛠️ PASO 1: Define tu array de razones
const razones = [
  {
    icono: '🏆',
    titulo: 'Calidad garantizada',
    descripcion: 'Ofrecemos soluciones confiables y ejecutadas con detalle en cada proyecto.',
  },
  {
    icono: '⚡',
    titulo: 'Entrega rápida',
    descripcion: 'Respetamos plazos ajustados sin renunciar a la precisión y claridad.',
  },
  {
    icono: '🤝',
    titulo: 'Atención personalizada',
    descripcion: 'Acompañamos su proceso con propuestas hechas a la medida de su negocio.',
  },
  {
    icono: '💡',
    titulo: 'Ideas innovadoras',
    descripcion: 'Aplicamos estrategias creativas y tecnología para maximizar sus resultados.',
  },
];

function WhyUs() {
  return (
    <section id="nosotros" className="py-5">
      <div className="container">
        <div className="row align-items-center g-5">

          {/* Columna izquierda: imagen */}
          <div className="col-lg-5">
            <img
              src="https://placehold.co/500x400/1a1e2e/f5c518?text=VoltTec"
              alt="Equipo de trabajo"
              className="img-fluid rounded shadow"
            />
          </div>

          {/* Columna derecha: texto y razones */}
          <div className="col-lg-7">
            <h2 className="section-titulo mb-3">¿Por qué elegirnos?</h2>

            <p className="text-muted mb-4">
              Somos una empresa dedicada a brindar soluciones confiables y modernas para su negocio.
              Nuestro enfoque combina calidad técnica con acompañamiento cercano en cada etapa del proyecto.
            </p>
            <p className="text-muted mb-4">
              Elegirnos significa contar con un equipo comprometido, creativo y orientado al resultado.
              Trabajamos para que su inversión genere valor real y un servicio sencillo de entender.
            </p>

            {/* 🛠️ PASO 2: Renderiza las razones con .map() */}
            <ul className="list-unstyled">
              {razones.map((razon, index) => (
                <li key={index} className="mb-4 d-flex align-items-start">
                  <span className="fs-3 me-3">{razon.icono}</span>
                  <div>
                    <h5 className="mb-1">{razon.titulo}</h5>
                    <p className="mb-0 text-muted">{razon.descripcion}</p>
                  </div>
                </li>
              ))}
            </ul>
          </div>

        </div>
      </div>
    </section>
  );
}

export default WhyUs;